...
import agenciaviajes.acceso.ServicioServidorCentral;
...
public class GestorClientes extends AModel {
    ...
    public GestorClientes() {
        ...
        servidor = new ServicioServidorCentral();
    ...
    public int getTotalHombres()throws ClassNotFoundException, SQLException{
        conector.conectarse();
        conector.crearConsulta("SELECT count(*) FROM clientes Where id='Masculino'");

        int hombres = 0;
        if (conector.getResultado().next()) {
           hombres= conector.getResultado().getInt(1);
        }
        conector.desconectarse();
        return hombres;
    }
